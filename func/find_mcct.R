...
nexus_metacommment <- function(phy, nodelabels=NULL, file=NULL, return=FALSE) {
	get_node_depths <- function(phy) {
		n <- ape::Ntip(phy) + 1
		h <- ifelse(seq(ape::Nnode(phy) + n - 1) == n, 0, NA)
		while (any(is.na(h))) {
			e <- which(phy$edge[,1] %in% n)
			n <- phy$edge[e,2]
			h[n] <- h[phy$edge[e,1]] + phy$edge.length[e]
		}
		return(round(abs(h - max(h)), 8))
	}
	ntip <- ape::Ntip(phy)
	nnode <- ape::Nnode(phy)
	tips <- phy$tip.label
	...
	if (!is.null(phy$edge.length) & ape::is.rooted(phy)) {
		depths <- get_node_depths(phy)
		node_labels <- paste0(node_labels, "|", "depth=", depths[-seq(ntip)])	
	}
	...
	if (!is.null(phy$edge.length)) {
		edge_labels <- paste0("[&length=", round(phy$edge.length, 8))
	} else {
		edge_labels <- rep("[&", ape::Nedge(phy))
	}
	...
	if (any(which_length)) {
		...
		edge_labels <- paste0(edge_labels, ",", length_hpd)
	}
	...
	for (i in seq(ntip)) {
		pattern <- paste0(tips[i], "[[:punct:]]{1}")
		m <- regexpr(pattern, newick)
