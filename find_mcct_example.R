...
mcct_comanc <- find_mcct(trees=nann, file="results/Nannomys_mcct_ca.tree", monophyletic=FALSE, method="mean", return=TRUE)
...
mcct_median <- find_mcct(trees=nann, file="results/Nannomys_mcct_med.tree", monophyletic=TRUE, method="median", return=TRUE)

# the second most probable topology (M. "Nyika" sister to the rest) used as a target to calculate mean common ancestor node heights
...
# the same, but returning nexus file with labels readable by FigTree 
find_mcct(trees=bathy, file="results/Bathyergidae_dating_mcct.nex", monophyletic=FALSE, method="mean", return=FALSE, burnin=0.20, thinning=5, figtree=TRUE)
